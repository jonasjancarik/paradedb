...
fn collect_actual_loops(plan: &Value, loops: &mut Vec<i64>) {
    if let Some(node_type) = plan.get("Node Type").and_then(|v| v.as_str()) {
        if node_type == "Custom Scan" {
            if let Some(al) = plan.get("Actual Loops").and_then(|v| v.as_i64()) {
                loops.push(al);
            }
        }
    }
...
fn collect_actual_rows(plan: &Value, rows: &mut Vec<i64>) {
    if let Some(node_type) = plan.get("Node Type").and_then(|v| v.as_str()) {
        if node_type == "Custom Scan" {
            if let Some(ar) = plan.get("Actual Rows").and_then(|v| v.as_i64()) {
                rows.push(ar);
            }
        }
    }
...
#[rstest]
fn parallel_topn_partition_baseline_no_parallel(mut conn: PgConnection) {
    ...
    // Verify exec methods are TopN
    let mut methods = Vec::new();
    collect_exec_methods(root, &mut methods);
    eprintln!("Exec methods: {methods:?}");
}

/// Verify that a parallel TopN query over a partitioned table:
...
#[rstest]
fn parallel_topn_partition_early_term_asc(mut conn: PgConnection) {
    if pg_major_version(&mut conn) < 17 {
        return;
    }
...
#[rstest]
fn parallel_topn_partition_early_term_desc(mut conn: PgConnection) {
    if pg_major_version(&mut conn) < 17 {
        return;
    }
...
#[rstest]
fn parallel_topn_partition_early_term_worst_case(mut conn: PgConnection) {
    if pg_major_version(&mut conn) < 17 {
        return;
    }
